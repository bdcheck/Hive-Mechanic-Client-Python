...
def post_request_with_retries(url, payload, logger, max_retry_duration=120, initial_retry_duration=3.75, server_timeout=60):
    query = None
    last_error = None
...
class HiveClient(object):
    ...
    def issue_commands(self, commands, player=None, endpoint=None):
        payload = {
            'token': self.token,
            'player': player
        }
    ...
    def fetch_variable(self, variable_name, player=None, default=None, scope='game'):
        ...
        logging.info('%s = %s' % (self.fetch_url(), str(response.json())))
        
        if 'value' in response.json():
            logging.info('%s = %s' % (variable_name, str(response.json()['value'])))
            
            return response.json()['value']

        logging.info('Unable to retrieve value for %s from %s' % (variable_name, str(self.fetch_url())))
